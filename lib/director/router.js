...
function dispatch (method, path, routes, fns, regexp, delim) {
  //
  // Clone the array so that we can explore multiple paths
  // simultaneously.
  //
  fns = fns.slice(0);
  ...
  for (var r in routes) {
    ...
    if (routes.hasOwnProperty(r)) {
      ...
      // current: '' + '/' + 'foo' = '/foo'
      // exact:  ['/foo']
      // approx: ['/foo/bar/buzz', '/bar/buzz']
      //
      current = regexp + delim + r;
      exact   = path.match(new RegExp('^' + current));
      approx  = path.match(new RegExp('^' + current + '(.*)?'));
      
      eyes.inspect(current, 'current');
      eyes.inspect(exact, 'exact');
      eyes.inspect(approx, 'approx');
      console.log();
      ...
      //
      if (exact && exact[0] === '/' && approx.length > 1 
        && '([a-zA-Z0-9-]+)' in routes) {
        continue;
      }
      ...
      if (dispatch.call(this, method, path, routes[r], fns, current, delim)) {
        match = true;
        break;
      }
    }
...
Router.prototype.dispatch = function (method, path, req, res) {
  ...
  return dispatch.call(thisArg, method, path, this.routes, [], '', this.delimiter);
};
