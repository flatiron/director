...
Router.prototype.dispatch = function (method, path, callback) {  
  ...
  function updateAndInvoke() {
    self.last = fns.after;
    self.invoke(runlist, {method: method, path: path}, callback);    
  }
  ...
  runlist = (this.every && this.every.before ? [this.every.before].concat(_flatten(fns)) : _flatten(fns));
  ...
  if (after && after.length > 0 && invoked) {
    if (this.async) {
      this.invoke(after, {method: method, path: path}, updateAndInvoke);
    }
    else {
      this.invoke(after, {method: method, path: path});
      updateAndInvoke();
    }
