...
function _every (arr, iterator) {
  for (var i = 0, valid = true; i < arr.length && valid; i += 1) {
...
Router.prototype.invoke = function (fns, thisArg, callback) {
  if (this.async) {
    ...
    _every(fns, function recursion(fn) {
      if (fn instanceof Array) {
        _every(fn, recursion);
      } else {
        fn.apply(thisArg, fns.captures || null);
      }
    });
  }
};
  ...
  for (var r in routes) {
    ...
    if (routes.hasOwnProperty(r) && !this._methods[r]) {
      ...
      if (next.matched) {
        ...
        //
        // **Remark:** Need a browser-capable version of `Array.prototype.filter`
        //
        fns = fns.concat(
          [routes[r].before].filter(Boolean),
          next, 
...
Router.prototype.insert = function (method, path, route, parent) {
  ...
  if (wrappers[method]) {
    route = wrappers[method](route);
  }

  if (parent[part] && !isArray && parentType == 'object') {
    methodType = typeof parent[part]['on'];

    switch (methodType) {
      case 'function': 
        parent[part]['on'] = [parent[part]['on'], route];
        return;
      case 'object':
        parent[part]['on'].push(route)
        return;
      case 'undefined':
        parent[part]['on'] = route;
        return;
    }
  }
